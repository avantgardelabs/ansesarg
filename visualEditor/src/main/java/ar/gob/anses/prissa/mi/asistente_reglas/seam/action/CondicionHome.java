...
//	}
	...
	public boolean encontrarDuplicado(){
		...
		boolean band = false;
		for (Iterator iterator = instance.getCondicionFormulaList().iterator(); iterator.hasNext(); ) {
		CondicionFormula condicionFormula2 = (CondicionFormula) iterator.next();
		
		if(condformulaSelected.equals(condicionFormula2)){
		band = true;
		break;
		}
		}
	...
	public void wire() {
		...
		try {
			if (this.instance.getAtributo().getTipoCarga().equals("LOGICO"))
				log.info("wire2");
					if (this.instance.getRegla()!=null || this.instance.getReglaPsC()!=null) {
						log.info("wire2.1");
					...
					if (this.instance.getRegla()==null && this.instance.getReglaPsC()==null && this.instance.getFuncion()==null) {
						this.setTipoCarga("REG");
	...
	public void addLiteral() {
		...
		if(props == null){
			...
			if(url != null) {
				try {
					...
					facesMessages.add("No fue posible setear el valor del properties.");
					return;
				}
			}
		}
		if(esVacio()){
			...
			if (esTexto()) currentLiteral = this.currentLiteral;
			...
			if (esNumero()){
				...
				if (operadorLogico.equals("BETWEEN"))
					if (cotaInf > cotaSup ) {
						facesMessages.add("Orden de los limites de comparacion incorrectos");
						return;
					}
	
				if ((this.auxMenorAUnValor!=null) && (!this.auxMenorAUnValor.equals("")))
					currentLiteral = cotaInf + ":" + cotaSup;
				else
					currentLiteral = this.currentLiteralNumero;
			}
			
			if (esBooleano()) currentLiteral = this.currentLiteralBoolean;
			
			if (esFecha()) {
				String fechaSup="", fechaInf="";
				Calendar cal = Calendar.getInstance();
				cal.setTime(getCurrentLiteralFecha());
				fechaInf =  cal.get(Calendar.DATE) + "/" + (cal.get(Calendar.MONTH) + 1) + "/" + cal.get(Calendar.YEAR); 
				if (operadorLogico.equals("BETWEEN")) {
					if (auxMenorAUnaFecha==null) {
						facesMessages.add("Debe especificar una fecha limite superior");
						return; 
					}
					if (this.currentLiteralFecha.before(auxMenorAUnaFecha)){
						Calendar aux = Calendar.getInstance();
						aux.setTime(auxMenorAUnaFecha);
						fechaSup = ":" + aux.get(Calendar.DATE) + "/" + (aux.get(Calendar.MONTH) + 1 ) + "/" + aux.get(Calendar.YEAR);
					}
					else {
						facesMessages.add("Orden de las fechas limite incorrecto");
						return;
					}
				}
		...
		for(Literal lit : this.instance.getLiterales()){
			if(lit.getNombre().trim().equals(currentLiteral.trim()) && lit.getOperadorLogico().equals(this.operadorLogico)) {
	...
	public void setModificado(boolean modificado) {
		...
		try {
			log.info("tipo del atributo: " + this.instance.getAtributo().getTipoDato() + " y su tpo de carga es: " + this.instance.getAtributo().getTipoCarga());
		}catch (Exception e) {}
	...
	public String remove() {
		...
		List<TablaDecision> reglasUsanConds = (List<TablaDecision>)query.getResultList();
		
		if(reglasUsanConds.size() != 0){
			log.info("No se puede eliminar la condicion debido a que esta siendo usada por una regla");
			facesMessages.add("No se puede eliminar la condicion debido a que esta siendo usada por una regla");
			return null;
		}
	...
	public String persist(){
		...
		for(Condicion c: condiciones){
			if((c.getNombre().toUpperCase().trim().equals(this.instance.getNombre().toUpperCase().trim()) && (c.getId()!=this.instance.getId()))){
				facesMessages.add("El nombre de la condicion ya esta siendo utilizado. Modifiquelo para guardar");
				return null;
			}
		}
		...
		if (this.instance.getAtributo().getTipoCarga().equals("FISICO")) {
			...
			facesMessages.add("La condicion debe contenter al menos un literal");
			return null;
		}
		else {
	...
	public List<Atributo> getAtributosEntidad2() {
		...
		//log.debug("Buscando los Atributos de entidad 2 " + this.instance.getEntidad().getNombre());
		...
		Query query = getPersistenceContext().createQuery("" +
				...
				.setParameter("ent", entidadAtributo2).setParameter("tDato", instance.getAtributo().getTipoDato());
		
		this.AtributosEntidad2 = query.getResultList();
		
		
		log.debug("Cantidad de atributos de la entidad2 " + AtributosEntidad2.size() + " Entidad Consultada: " + this.entidadAtributo2.getNombre());
	...
	public String getCreatedMessage() { return this.isManaged() ? "Condicion #{condicionHome.instance.nombre} actualizada exitosamente" : "Nueva condicion #{accionHome.instance.nombre} creada exitosamente"; }
    public String getUpdatedMessage() { return "Condicion #{condicionHome.instance.nombre} actualizada exitosamente"; }
    public String getDeletedMessage() { return "Condicion #{condicionHome.instance.nombre} borrada exitosamente"; }
...
//			instance.getCondicionAtributo().setAtributo2(atributo2);
	...
	public void buscarFunciones(){
		...
		Query query = entityManager.createQuery("" +
				...
				.setParameter("patron", "%"+this.nombreFuncion.toUpperCase()+"%")
				.setParameter("tipo",this.instance.getAtributo().getTipoDato().toUpperCase());
	...
	public String agregarCondicionAtributo(){
		...
		boolean band = false;
		for (Iterator iterator = instance.getCondicionAtributoList().iterator(); iterator.hasNext(); ) {
		CondicionAtributo condicionAtributo2 = (CondicionAtributo) iterator.next();
		
		if(condicionAtributo.getAtributo2().equals(condicionAtributo2.getAtributo2()))
			band = true;
		}
		if(band==false)
